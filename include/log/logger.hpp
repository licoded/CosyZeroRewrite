...
#define LOG_TRACE(...)                                     \
    do                                                     \
    {                                                      \
        if (auto lg = logger::Logger::instance().logger()) \
...
#define LOG_ERROR(...)                                     \
    ...
    {                                                      \
        if (auto lg = logger::Logger::instance().logger()) \
            lg->error(__VA_ARGS__);                        \
    } while (0)
#define LOG_CRITICAL(...)                                  \
    do                                                     \
    {                                                      \
        if (auto lg = logger::Logger::instance().logger()) \
            lg->critical(__VA_ARGS__);                     \
    } while (0)
...
#define NOP_LOG_INFO(...)                                            \
    do                                                               \
    {                                                                \
        if (auto lg = logger::Logger::instance().no_perfix_logger()) \
        {                                                            \
            lg->info(__VA_ARGS__);                                   \
        }                                                            \
    } while (0)
#define NOP_LOG_ERROR(...)                                           \
    do                                                               \
    {                                                                \
        if (auto lg = logger::Logger::instance().no_perfix_logger()) \
        {                                                            \
...
#define LOG_FLUSH()                                                  \
    ...
    {                                                                \
        if (auto lg = logger::Logger::instance().logger())           \
            lg->flush();                                             \
        if (auto ol = logger::Logger::instance().no_perfix_logger()) \
            ol->flush();                                             \
    } while (0)
