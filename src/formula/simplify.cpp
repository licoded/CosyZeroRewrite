 ...
 */
void collect_binary_terms(Formula *f, std::set<Formula *> &terms, Formula::OpType op)
{
    if (!f)
        return;
 ...
 */
bool has_complementary_literals(const std::set<Formula *> &terms, FormulaPool &pool)
{
 ...
 */
Formula *rebuild_chain(FormulaPool &pool, const std::set<Formula *> &terms, Formula::OpType op)
{
    if (terms.empty())
    {
...
Formula *simplify_until(FormulaPool &pool, Formula *left, Formula *right)
{
    ...
    {
        return pool.create_next(pool.create_until(left->left(), right->left()));
    }
