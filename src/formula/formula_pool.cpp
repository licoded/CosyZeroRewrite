...
void FormulaPool::validate_variable_name(const std::string &name)
{
    ...
    if (name.length() > MAX_VAR_NAME_LENGTH)
    {
        throw InvalidVariableNameException(name, "exceeds maximum length of " + std::to_string(MAX_VAR_NAME_LENGTH));
    }
    ...
    if (!std::regex_match(name, pattern))
    {
        throw InvalidVariableNameException(name, "must match pattern [a-zA-Z_][a-zA-Z0-9_]*");
    }
...
size_t FormulaPool::compute_hash(Formula::OpType op, Formula *left, Formula *right, int var_id)
{
...
bool FormulaPool::FormulaEqual::operator()(Formula *a, Formula *b) const noexcept
{
    ...
    return a->hash() == b->hash() && a->op() == b->op() && a->left() == b->left() && a->right() == b->right()
           && a->var_id() == b->var_id();
}

// ========== Validation ==========

void FormulaPool::validate_create(Formula::OpType op, Formula *left, Formula *right, int var_id)
{
    switch (op)
    {
...
void FormulaPool::declare_variables(const std::vector<std::string> &outputs, const std::vector<std::string> &inputs)
{
    ...
    for (const auto &name : inputs)
    {
        if (!seen.insert(name).second)
        {
            throw std::invalid_argument("Duplicate input (also in outputs): " + name);
        }
    }
...
Formula *FormulaPool::create(Formula::OpType op, Formula *left, Formula *right, int var_id)
{
