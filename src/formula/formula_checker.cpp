...
bool FormulaChecker::check_xnf_recursive(Formula* f, std::unordered_set<Formula*>& visited, bool inside_next) {
    if (!f) return true;
    if (visited.count(f)) return true;
    visited.insert(f);
    ...
    switch (f->op()) {
        ...
        case Formula::OpType::Or:
            ...
            return check_xnf_recursive(f->left(), visited, false) &&
                   check_xnf_recursive(f->right(), visited, false);
...
bool FormulaChecker::is_temporal(Formula* f) {
    ...
    switch (f->op()) {
        ...
        case Formula::OpType::Or:
        case Formula::OpType::Not:
            return is_temporal(f->left()) ||
                   (f->right() && is_temporal(f->right()));
