...
static std::string solver_to_dot(const OnTheFlyGameSolver &solver, StateIdMap &id_map)
{
  ...
  // Sys moves: blue solid lines | Env moves: red dashed lines
    ...
    for (const auto &pair : id_map.to_id)
    {
        ...
        std::string attrs
            = fmt::format("[shape={}, fillcolor={}, color={} label=\"{}\"]", shape, fillcolor, color, label);
            ...
            if (from_is_sys)
            {
                ...
                if (succ.system_chosen_output.has_value() && from.dfa_state)
                {
                    attrs += fmt::format(", label=\"sys={}\"",
                                         id_map.get_assignment_label(
                                             succ.system_chosen_output.value(), true, &from.dfa_state->prop_atoms()));
                }
                attrs += "]";
            }
            else
            {
                ...
                if (succ.environment_chosen_input.has_value() && from.dfa_state)
                {
                    attrs += fmt::format(", label=\"env={}\"",
                                         id_map.get_assignment_label(succ.environment_chosen_input.value(),
                                                                     false,
...
void TraceExporter::begin_stage(const std::string &stage_type, const std::string &description)
{
    if (!enabled_)
        return;
...
void TraceExporter::end_stage()
{
    ...
    double duration_ms = std::chrono::duration<double, std::milli>(end_time - stage_start_time_).count();

    LOG_DEBUG("TraceExporter: ended stage {} ({}), duration: {}ms",
...
void TraceExporter::end_sub_step(bool include_graph)
{
    ...
    if (current_sub_step_)
    {
        ...
        current_sub_step_->metrics.duration_ms
            = std::chrono::duration<double, std::milli>(end_time - step_start_time_).count();
    }
    ...
    if (enabled_ && current_stage_index_ >= 0)
    {
        ...
        if (!stage.sub_steps.empty())
        {
            ...
            LOG_DEBUG("TraceExporter: ended step {} ({}), duration: {}ms",
                      step_counter_,
                      step.description,
...
void TraceExporter::set_graph_dot(const std::string &dot, size_t num_nodes, size_t num_edges)
{
    if (current_sub_step_)
    {
...
void TraceExporter::add_highlight_node(const std::string &node_id, HighlightType type)
{
    if (current_sub_step_)
    {
        switch (type)
...
void TraceExporter::add_highlight_nodes(const std::vector<std::string> &node_ids, HighlightType type)
{
                                  ...
                                  const SubStepHighlights *highlights)
{
    ...
    {
        if (pair.second == StateClass::Swin)
            swin++;
        else if (pair.second == StateClass::Ewin)
            ewin++;
        else
            unknown++;
    }
...
void TraceExporter::finalize(bool realizable)
{
    ...
    auto end_time = std::chrono::steady_clock::now();
    summary.duration_ms = std::chrono::duration<double, std::milli>(end_time - start_time_).count();
    ...
    for (const auto &stage : stages_)
    {
        ...
        if (!found)
        {
            summary.stages_summary.push_back({stage.stage_type, static_cast<int>(stage.sub_steps.size())});
        }
    }
...
std::string TraceExporter::format_assignment_label(const GameState &state, const GameState &succ)
{
    ...
    {
        true_vars.insert(succ.system_chosen_output.value().begin(), succ.system_chosen_output.value().end());
    }
    else if (!is_sys_move && succ.environment_chosen_input.has_value())
    {
        true_vars.insert(succ.environment_chosen_input.value().begin(), succ.environment_chosen_input.value().end());
    }
    ...
    if (state.dfa_state)
    {
        for (const auto *phi : state.dfa_state->prop_atoms())
        {
            if (phi && phi->op() == formula::Formula::OpType::Literal)
            {
                ...
                else if (!is_sys_move && var_id >= num_outputs && var_id < static_cast<int>(var_names.size()))
                {
                    relevant_vars.push_back(var_id);
                }
            }
...
nlohmann::json TraceExporter::highlights_to_json(const SubStepHighlights &h) const
{
    ...
    if (!h.new_nodes.empty())
        j["new_nodes"] = h.new_nodes;
    if (!h.scc_nodes.empty())
        j["scc_nodes"] = h.scc_nodes;
    if (!h.pending_nodes.empty())
        j["pending_nodes"] = h.pending_nodes;
    if (!h.updated_nodes.empty())
        j["updated_nodes"] = h.updated_nodes;
    if (!h.attractor_nodes.empty())
        j["attractor_nodes"] = h.attractor_nodes;
    if (!h.scc_id.empty())
        j["scc_id"] = h.scc_id;
...
void TraceExporter::collect_state_data(SubStepGraphData &graph_data, const OnTheFlyGameSolver &solver)
{
    ...
    for (const GameState &state : iter::imap(
             [](const auto &p) {
                 return p.first;
             },
             successors))
