...
std::string GameState::to_string() const
{
    ...
    oss << "{dfa=" << dfa_state << ", player=" << (player == Player::System ? "Sys" : "Env");
    if (system_chosen_output.has_value())
    {
        oss << ", out={" << fmt::format("{}", fmt::join(system_chosen_output.value(), ",")) << "}";
    }
    if (environment_chosen_input.has_value())
    {
        oss << ", in={" << fmt::format("{}", fmt::join(environment_chosen_input.value(), ",")) << "}";
    }
    oss << "}";
    return oss.str();
}
...
OnTheFlyGameSolver::OnTheFlyGameSolver(formula::Formula *phi, formula::FormulaPool &pool)
    : pool_(pool),
    ...
    if (state.player == Player::System)
    {
        ...
        if (enable_bdd_filtering_ && bdd_manager_)
        {
            ...
            std::vector<Assignment> safe_moves
                = bdd_manager_->enumerate_safe_sys_moves(state.dfa_state, relevant_output_var_ids, pool_);
...
static bool formula_empty_string_accepting(formula::Formula *f)
{
    ...
    switch (f->op())
    {
        ...
        case formula::Formula::OpType::And:
            return formula_empty_string_accepting(f->left()) && formula_empty_string_accepting(f->right());
...
bool OnTheFlyGameSolver::is_empty_string_accepting(automata::TableauState *q) const
{
    ...
    if (!phi)
        return true; // Empty phi is vacuously true
...
bool OnTheFlyGameSolver::classify_scc(const std::vector<GameState> &scc)
{
    ...
    std::unordered_map<GameState, std::unordered_set<GameState, GameStateHash>, GameStateHash, GameStateEqual>
        predecessors;
    ...
    for (const auto &s : scc)
    {
        ...
        LOG_DEBUG("classify_scc: state={}, phi={}, esa={}",
                  s.to_string(),
    ...
    while (changed)
    {
        ...
        for (const GameState &s : tmpSet)
        {
            ...
            { // sys move → env states
                ...
                if (all_sys_moves_ewin && classification_.count(e) && classification_[e] != StateClass::Ewin)
                {
    ...
    for (const auto &pair : successors_)
    {
        ...
        if (state.player == Player::System)
        {
            ...
            if (cls == StateClass::Swin)
            {
                ...
                {
                    LOG_WARN(
                        "Propagation violation: System state {} is Swin but has no Swin successor (has {} successors)",
                        state.to_string(),
                        succs.size());
                    violations++;
                }
            ...
            else if (cls == StateClass::Ewin)
            {
                ...
                for (const auto &succ : succs)
                {
                    ...
                    if (succ_cls == classification_.end() || succ_cls->second != StateClass::Ewin)
                    {
                        all_ewin = false;
                        LOG_WARN("Propagation violation: System state {} is Ewin but successor is not Ewin",
                                 state.to_string());
                        break;
                    }
                }
        ...
        else
        {
            ...
            if (cls == StateClass::Swin)
            {
                ...
                for (const auto &succ : succs)
                {
                    ...
                    {
                        all_swin = false;
                        LOG_WARN("Propagation violation: Environment state {} is Swin but not all successors are Swin",
                                 state.to_string());
                        break;
                    }
                }
            ...
            else if (cls == StateClass::Ewin)
            {
                ...
                {
                    LOG_WARN("Propagation violation: Environment state {} is Ewin but has no Ewin successor (has {} "
                             "successors)",
                             state.to_string(),
                             succs.size());
                    violations++;
...
void OnTheFlyGameSolver::enable_trace(const std::string &output_dir)
{
    ...
    trace_exporter_ = TraceExporter(original_formula_, pool_, trace_dir);
...
{
    return trace_exporter_.is_enabled();
}
