...
TableauState::TableauState(formula::Formula *phi, formula::Formula *xnf_phi, FormulaSet prop_atoms)
    : phi_(phi), xnf_phi_(xnf_phi), prop_atoms_(std::move(prop_atoms)), hash_(phi ? phi->hash() : 0)
{
}
 ...
 */
formula::Formula *formula_progression(formula::Formula *phi, const Assignment &sigma, formula::FormulaPool &pool)
{
    if (!phi)
    ...
    switch (op)
    {
        ...
        case formula::Formula::OpType::Literal:
            // fp(p, σ) = tt if p ∈ σ, else ff
            return sigma.count(phi->var_id()) > 0 ? pool.create_true() : pool.create_false();
        ...
        case formula::Formula::OpType::Not:
        {
            ...
            {
                return sigma.count(child->var_id()) == 0 ? pool.create_true() : pool.create_false();
            }
            ...
            if (child_prog->is_true())
                return pool.create_false();
            if (child_prog->is_false())
                return pool.create_true();
        ...
        case formula::Formula::OpType::And:
        {
            ...
            if (left_prog->is_false() || right_prog->is_false())
                return pool.create_false();
            // (true & true) → true
            if (left_prog->is_true() && right_prog->is_true())
                return pool.create_true();
            // (true & φ) → φ, (φ & true) → φ
            if (left_prog->is_true())
                return right_prog;
            if (right_prog->is_true())
                return left_prog;
        ...
        case formula::Formula::OpType::Or:
        {
            ...
            if (left_prog->is_false() && right_prog->is_false())
                return pool.create_false();
            // (false | φ) → φ, (φ | false) → φ
            if (left_prog->is_false())
                return right_prog;
            if (right_prog->is_false())
                return left_prog;
...
formula::Formula *TableauState::next_phi(const Assignment &assignment, formula::FormulaPool &pool) const
{
    // DEBUG: Log input
...
bool TableauState::is_temporal(formula::Formula *f)
{
    if (!f)
        return false;
    auto op = f->op();
    return op == formula::Formula::OpType::Next || op == formula::Formula::OpType::Until
...
TableauState *TableauStatePool::get_or_create(formula::Formula *phi, formula::FormulaPool &pool)
{
    ...
    auto actual_state = std::unique_ptr<TableauState>(new TableauState(phi, xnf_phi, std::move(prop_atoms)));
...
AssignmentGenerator::AssignmentGenerator(int num_variables) : num_variables_(num_variables) {}
...
std::string AssignmentGenerator::to_string(const Assignment &a, const formula::FormulaPool &pool)
{
