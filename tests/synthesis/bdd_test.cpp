...
void test_rm_next_basic()
{
    ...
    Formula *phi2 = pool.create_and(pool.create_variable("p0"), pool.create_next(pool.create_variable("p1")));
    ...
    Formula *phi3 = pool.create_or(pool.create_next(pool.create_variable("p0")), pool.create_variable("p1"));
    ...
    Formula *phi4 = pool.create_not(pool.create_next(pool.create_variable("p0")));
...
void test_xnf_phi_vs_prop_atoms()
{
    ...
    Formula *xnf_phi
        = pool.create_or(pool.create_and(pool.create_variable("p0"), pool.create_next(pool.create_variable("p1"))),
    ...
    if (rm->left()->is_literal() && rm->left()->var_id() == 0)
        has_p0 = true;
    if (rm->right()->is_literal() && rm->right()->var_id() == 2)
        has_p2 = true;
    // Also check the other side of OR
    if (rm->left()->is_literal() && rm->left()->var_id() == 2)
        has_p2 = true;
    if (rm->right()->is_literal() && rm->right()->var_id() == 0)
        has_p0 = true;
...
void test_nested_next()
{
    ...
    // X(X(p0)) → True → True
    Formula *phi = pool.create_next(pool.create_next(pool.create_variable("p0")));
...
void test_mixed_boolean_temporal()
{
    ...
    Formula *phi = pool.create_and(
        pool.create_or(pool.create_variable("a"), pool.create_next(pool.create_variable("b"))),
        pool.create_or(pool.create_next(pool.create_variable("c")), pool.create_not(pool.create_variable("b"))));
