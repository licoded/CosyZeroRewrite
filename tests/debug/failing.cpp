...
int main()
{
    ...
    std::cout << "\n=== Test 3: p1 & !p1 ===" << std::endl;
    {
        ...
        {
            std::cout << "  not_p1->left op=" << (int)not_p1->left()->op() << ", var_id=" << not_p1->left()->var_id()
                      << std::endl;
        }
        std::cout << "  and_f left op=" << (int)and_f->left()->op() << ", var_id=" << and_f->left()->var_id()
                  << std::endl;
        std::cout << "  and_f right op=" << (int)and_f->right()->op() << std::endl;
        if (and_f->right()->op() == formula::Formula::OpType::Not && and_f->right()->left())
        {
            std::cout << "  and_f->right->left op=" << (int)and_f->right()->left()->op()
                      << ", var_id=" << and_f->right()->left()->var_id() << std::endl;
        }
