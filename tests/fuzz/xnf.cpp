...
bool check_nnf_rules(Formula *f, std::string *error_msg = nullptr)
{
    ...
    switch (op)
    {
        ...
        case Formula::OpType::Release:
        {
            ...
            {
                if (error_msg)
                    *error_msg = "Left child: " + left_error;
                return false;
            }
            if (!right_ok)
            {
                if (error_msg)
                    *error_msg = "Right child: " + right_error;
                return false;
            }
            return true;
        }
...
bool is_xnf(Formula *f, FormulaPool &pool, std::string *error_msg = nullptr)
{
    ...
    switch (op)
    {
        ...
        case Formula::OpType::Or:
        {
            ...
            {
                if (error_msg)
                    *error_msg = "Left child invalid: " + left_error;
                return false;
            }
            if (!right_ok)
            {
                if (error_msg)
                    *error_msg = "Right child invalid: " + right_error;
                return false;
            }
            return true;
        }
        ...
        case Formula::OpType::Next:
        {
            ...
            {
                if (error_msg)
                    *error_msg = "Child of X violates NNF: " + child_error;
                return false;
            }
            return true;
        }
...
class RandomFormulaGenerator
{
  public:
    ...
    std::string generate() { return generate(config_.max_depth); }
    ...
    std::string generate(int depth)
    {
        ...
        switch (choice)
        {
            ...
            case 3:
                return "!" + generate_leaf(); // !literal
            case 4:
                return "(" + generate(depth - 1) + " & " + generate(depth - 1) + ")"; // And
            case 5:
                return "(" + generate(depth - 1) + " | " + generate(depth - 1) + ")"; // Or
            case 6:
                return "X(" + generate(depth - 1) + ")"; // Next
            case 7:
                return "(" + generate(depth - 1) + " U " + generate(depth - 1) + ")"; // Until
            case 8:
                return "(" + generate(depth - 1) + " R " + generate(depth - 1) + ")"; // Release
            default:
                return generate_leaf();
        }
    }
...
struct FuzzStats {
    ...
    void print_summary() const
    {
        ...
        if (total > 0)
        {
            std::cout << "XNF validity rate:    " << std::fixed << std::setprecision(2) << (100.0 * xnf_valid / total)
                      << "%" << std::endl;
            std::cout << "Average time:        " << std::fixed << std::setprecision(2) << (total_time_ms / total)
                      << " ms" << std::endl;
        }
        std::cout << "========================================" << std::endl;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2)
    {
        ...
        catch (...)
        {
            ...
            std::cerr << "Usage: " << argv[0] << " [num_formulas] [max_depth] [num_variables] [print_count]"
                      << std::endl;
            return 1;
        }
    }
    ...
    for (int i = 0; i < config.num_formulas; ++i)
    {
        ...
        {
            std::cout << "Progress: " << (i + 1) << "/" << config.num_formulas << " (valid: " << stats.xnf_valid
                      << ", invalid: " << stats.xnf_invalid << ", parse errors: " << stats.parse_errors << ")"
                      << std::endl;
        }
    }
