...
struct FuzzStats {
    ...
    void print_summary() const
    {
        ...
        if (total > 0)
        {
            std::cout << "NNF validity rate:    " << std::fixed << std::setprecision(2) << (100.0 * nnf_valid / total)
                      << "%" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < config.num_formulas; ++i)
    {
        ...
        {
            std::cout << "Progress: " << (i + 1) << "/" << config.num_formulas << " (valid: " << stats.nnf_valid
                      << ", invalid: " << stats.nnf_invalid << ", parse errors: " << stats.parse_errors << ")"
                      << std::endl;
        }
    }
