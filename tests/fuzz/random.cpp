...
class RandomFormulaGenerator
{
  ...
  public:
    RandomFormulaGenerator(int max_var_id = 10, int seed = 0)
        : rng_(seed), op_dist_(0, 8), var_dist_(0, max_var_id), depth_dist_(1, 6)
    {
    ...
    Formula *generate(FormulaPool &pool, int max_depth) { return generate(pool, 0, max_depth); }
...
struct TestResult {
    ...
    void print(const std::string &name) const
    {
        std::cout << "[" << name << "] "
                  ...
                  << ", Time: " << total_time_ms << "ms" << std::endl;
    }
};
...
int main(int argc, char *argv[])
{
    ...
    return (parser_result.crashed > 0 || nnf_result.failed > 0 || xnf_result.failed > 0 || simplify_result.failed > 0)
               ? 1
               : 0;
}
