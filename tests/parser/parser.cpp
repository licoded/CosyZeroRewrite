...
TEST_CASE("Property: NNF preserves semantics", "[property][nnf]")
{
    ...
    std::vector<std::string> formulas
        = {"a", "!a", "a & b", "a | b", "!(a & b)", "!(a | b)", "!X(a)", "X(a)", "a U b", "a R b"};
...
TEST_CASE("Property: XNF preserves semantics", "[property][xnf]")
{
    ...
    std::vector<std::string> formulas = {"a", "X(a)", "a & b", "a | b", "a U b", "a R b", "X(a) U b", "X(a & b)"};
...
TEST_CASE("Property: Simplify preserves semantics", "[property][simplify]")
{
    ...
    std::vector<std::string> formulas
        = {"a & true", "a | false", "a & a", "!!a", "true & a", "false | a", "(a & b) & (b & a)"};
    ...
    for (const auto &formula_str : formulas)
    {
        ...
        REQUIRE(FormulaChecker::formula_size(simp_f) <= FormulaChecker::formula_size(f));
    }
}
...
TEST_CASE("Property: Simplify is idempotent", "[property][simplify]")
{
    ...
    std::vector<std::string> formulas = {"a", "a & b & c", "a | b | c", "!(!a)", "X(X(a))"};
...
TEST_CASE("Property: NNF is idempotent", "[property][nnf]")
{
    ...
    std::vector<std::string> formulas = {"a", "a & b", "a | b", "X(a)"};
